...
#-------------------------------------------------------MR Analysis--------------------------------------------------------------#
	...
	#-------------------------BMD-----------------------#
		...
		yx = read.table("index_snp_BMD_Biobank2_Rheumatoid_Arthritis_European_2014_nature_2018bmj.txt",header = T)
		dim(yx)
	...
	#-------------------------Fracture-----------------------#
		yx = read.table("index_snp_Fractures_Biobank2_Rheumatoid_Arthritis_European_2014_nature_2018bmj.txt",header = T)
		dim(yx)
		index.2 = which(as.numeric(yx$P.y)<(0.05/dim(yx)[1]))
		yx$SNP[index.2]
...
#--------------------------reverse------------------------------#	
		library(data.table)
		DD0=data.frame(fread(paste("BMD_Biobank2.txt",sep=""),head=T,sep="\t"))
		Exp = c("Rheumatoid_Arthritis_European_2014_nature")
		setwd("/data/yxh/data/summarydata")
		ax0 = data.frame(fread(paste("Rheumatoid_Arthritis_European_2014_nature.txt",sep = ""),head=T,sep='\t'))

		snp = as.character(read.table(paste("snp_BMD_Biobank2.txt",sep=""),head=T)[,1])
		#snp = as.character(read.table(paste("plink_",Exp[i],"_plink_0.1_clumped.txt",sep=""),head=T)$SNP)
...
#-------------------------------------UKB-------------------------------------#
	...
	#genotype
		...
		fwrite(data.frame(gx),"Genotype_ra_2018bmj.txt", row.names = F, col.names = T,quote = F, sep = "\t")
		
		# merge 33IDÓë48ID
		gx = data.frame(fread("Genotype_ra_2018bmj.txt", header = T))
